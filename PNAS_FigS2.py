# -*- coding: utf-8 -*-
"""
Created on Tue May 21 13:05:53 2024
...
# Construct the main diagonal with g_i values increased by 10%
main_diag = np.array([g1 * (1.1 ** i) for i in range(n)])

# Construct the tridiagonal matrix with weak-strong-weak pattern
upper_diag = np.array([-w if i % 2 == 0 else -s for i in range(n-1)])
lower_diag = np.array([-w if i % 2 == 0 else -s for i in range(n-1)])

# Construct the matrix beta
...
# # Manually set the initial condition for the first variable to 2.0
# x0[0] = 2.0
...
for i in range(n):
    plt.subplot(n, 1, i+1)
    plt.plot(time_first_500, x_first_500[i])
    plt.xlabel('Time')
    plt.ylabel(f'x_{i+1}')
    plt.title(f'Variation of x_{i+1} over the last 500 time units')
    plt.grid(True)
